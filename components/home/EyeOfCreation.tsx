...
// 700 was declared but never used below — dropping it saves a font file on the
// heaviest route the site has.
...
function computeVals(p: number, showScrollHint: boolean, isLight: boolean) {
  ...
  const focalRingBStyle: CSSProperties = {
    ...
    background: "repeating-conic-gradient(from 0deg, rgba(var(--eoc-accent), 0) 0deg 9deg, rgba(var(--eoc-accent), 0.55) 9deg 10deg)",
  ...
  const rimStyle: CSSProperties = {
    ...
    background: "conic-gradient(from 200deg, transparent 0deg, rgba(var(--eoc-accent), 0.85) 40deg, rgba(var(--eoc-accent), 0.95) 62deg, rgba(var(--eoc-accent), 0.7) 84deg, transparent 130deg)",
  ...
  const glassStyle: CSSProperties = {
    ...
    background: `radial-gradient(circle at 46% 42%, rgba(var(--eoc-glass), ${(0.14 + glassGlow * 0.3).toFixed(3)}) 0%, color-mix(in srgb, var(--eoc-glass-bg) 90%, transparent) 42%, var(--eoc-glass-ink) 78%)`,
    boxShadow: `inset 0 0 60px rgba(0,0,0,0.9), inset 0 0 30px rgba(var(--eoc-glass-3), ${(0.1 + glassGlow * 0.4).toFixed(3)})`,
  };
  const glassShimmerStyle: CSSProperties = {
    position: "absolute", left: "-30%", top: "-30%", width: "160%", height: "160%",
    background: "conic-gradient(from 0deg, transparent 0deg, rgba(var(--eoc-glow), 0.14) 30deg, transparent 70deg, transparent 360deg)",
  ...
  const sloganStyle: CSSProperties = {
    ...
    color: "rgba(var(--eoc-sky-3), 0.92)", textShadow: "0 0 22px rgba(var(--eoc-glow-2), 0.5)",
  ...
  const coreGlowStyle: CSSProperties = {
    ...
    background: "radial-gradient(circle, rgba(var(--eoc-glow), 0.35) 0%, rgba(var(--eoc-glass-2), 0.12) 30%, transparent 62%)",
  ...
  const beams = beamCfg.map((b, i) => {
    ...
    return {
      style: {
        ...
        background: "linear-gradient(to bottom, rgba(var(--eoc-beam), 0.55), rgba(var(--eoc-accent), 0.22) 42%, rgba(var(--eoc-accent), 0.06) 68%, transparent 84%)",
      ...
      flareStyle: {
        ...
        background: "radial-gradient(circle, rgba(255,255,255,0.98) 0%, rgba(var(--eoc-beam), 0.75) 20%, rgba(var(--eoc-accent), 0.35) 42%, transparent 66%)",
  ...
  const neons = neonCfg.map((n) => {
    ...
    return {
      style: {
        ...
        background: "rgba(var(--eoc-accent), 0.95)",
        boxShadow: "0 0 6px rgba(var(--eoc-accent), 0.9), 0 0 16px rgba(var(--eoc-accent), 0.7), 0 0 34px rgba(var(--eoc-accent), 0.5)",
        borderRadius: "2px",
  ...
  const lineColors = [
    "linear-gradient(90deg,rgb(var(--eoc-primary)),rgb(var(--eoc-soft)))", "linear-gradient(90deg,rgb(var(--eoc-soft)),rgb(var(--eoc-primary)))",
    "linear-gradient(90deg,rgb(var(--eoc-primary)),var(--eoc-navy))", "linear-gradient(90deg,var(--eoc-indigo),var(--eoc-indigo))",
    "linear-gradient(90deg,rgb(var(--eoc-primary)),rgb(var(--eoc-soft)))", "linear-gradient(90deg,rgb(var(--eoc-soft)),rgb(var(--eoc-primary)))",
  ];
    ...
    <div ref={scaffoldRef} style={{ position: "relative", height: `${LENGTH_VH[length]}vh`, background: v.stage.bg }}>
      ...
      >
        ...
        <div style={v.lensGroupStyle}>
          ...
          <div style={{ position: "absolute", left: "50%", top: "50%", width: "40vh", height: "40vh", marginLeft: "-20vh", marginTop: "-20vh", borderRadius: "50%", background: "radial-gradient(circle at 34% 28%, rgba(var(--eoc-sky-4), 0.16) 0%, transparent 34%)", pointerEvents: "none" }} />
        </div>

        {/* ===== REFLECTION PANELS ===== */}
        <div style={v.filmStyle}>
          <div style={{ position: "absolute", inset: 0, borderRadius: 14, overflow: "hidden", background: "linear-gradient(180deg, var(--eoc-navy-deep) 0%, var(--eoc-navy) 46%, var(--eoc-navy-mid) 100%)" }}>
            <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, height: "38%", background: "linear-gradient(180deg, transparent, rgba(var(--eoc-primary), 0.30))" }} />
            <div style={{ position: "absolute", left: 0, right: 0, bottom: "22%", height: 1, background: "rgba(var(--eoc-soft), 0.4)" }} />
            <div style={v.droneStyle}>
              <div style={{ position: "absolute", left: "50%", top: "50%", width: 120, height: 5, margin: "-2.5px 0 0 -60px", background: "linear-gradient(90deg, transparent, rgb(var(--eoc-soft)), transparent)", borderRadius: 3 }} />
              <div style={{ position: "absolute", left: 6, top: -4, width: 8, height: 8, borderRadius: "50%", background: "rgb(var(--eoc-primary))", boxShadow: "0 0 12px rgb(var(--eoc-primary))", animation: "eoc-rotorL 0.9s linear infinite" }} />
              <div style={{ position: "absolute", right: 6, top: -4, width: 8, height: 8, borderRadius: "50%", background: "rgb(var(--eoc-soft))", boxShadow: "0 0 12px rgb(var(--eoc-soft))", animation: "eoc-rotorL 0.9s linear infinite reverse" }} />
              <div style={{ position: "absolute", left: "50%", top: 8, width: 2, height: 130, marginLeft: -1, background: "linear-gradient(180deg, rgba(var(--eoc-soft), 0.55), transparent)", filter: "blur(1px)" }} />
            </div>
            ...
            <div dir="ltr" style={{ position: "absolute", left: 20, bottom: 16, ...labelStyle }}>
              ...
              <div style={{ fontSize: 11, letterSpacing: "0.4em", color: "rgba(var(--eoc-primary), 0.75)", marginTop: 6 }}>AERIAL CINEMATOGRAPHY</div>
            </div>
            <div style={{ position: "absolute", inset: 0, borderRadius: 14, boxShadow: "inset 0 0 60px rgba(var(--eoc-primary), 0.28), inset 0 0 0 1px rgba(var(--eoc-soft), 0.35)" }} />
          </div>
        </div>

        <div style={v.codeStyle}>
          <div style={{ position: "absolute", inset: 0, borderRadius: 14, overflow: "hidden", background: "linear-gradient(180deg, var(--eoc-navy-panel) 0%, var(--eoc-navy) 100%)", fontFamily: FONT_MONO, padding: "26px 30px" }}>
            <div style={{ position: "absolute", top: 14, left: 16, display: "flex", gap: 7 }}>
              <div style={{ width: 11, height: 11, borderRadius: "50%", background: "var(--eoc-navy)", boxShadow: "inset 0 0 0 1px rgba(var(--eoc-soft), 0.4)" }} />
              <div style={{ width: 11, height: 11, borderRadius: "50%", background: "rgb(var(--eoc-primary))" }} />
              <div style={{ width: 11, height: 11, borderRadius: "50%", background: "rgb(var(--eoc-soft))" }} />
            </div>
            ...
            <div dir="ltr" style={{ position: "absolute", left: 30, bottom: 16, ...labelStyle }}>
              ...
              <div style={{ fontSize: 11, letterSpacing: "0.4em", color: "rgba(var(--eoc-primary), 0.75)", marginTop: 6 }}>EXPERIENCE ENGINEERING</div>
            </div>
            <div style={{ position: "absolute", inset: 0, borderRadius: 14, boxShadow: "inset 0 0 60px rgba(var(--eoc-primary), 0.20), inset 0 0 0 1px rgba(var(--eoc-soft), 0.3)" }} />
          </div>
        </div>

        <div style={v.photoStyle}>
          <div style={{ position: "absolute", inset: 0, borderRadius: 14, overflow: "hidden", background: "radial-gradient(90% 80% at 50% 30%, var(--eoc-navy-mid) 0%, var(--eoc-navy) 58%, var(--eoc-navy-ink) 100%)" }}>
            <div style={{ position: "absolute", left: "50%", top: "-10%", width: "60%", height: "120%", marginLeft: "-30%", background: "linear-gradient(180deg, rgba(var(--eoc-soft), 0.26), transparent 70%)", filter: "blur(14px)", transform: "perspective(400px) rotateX(20deg)", transformOrigin: "top" }} />
            <div style={{ position: "absolute", left: "50%", bottom: "8%", width: 120, height: 220, marginLeft: -60, borderRadius: "60px 60px 40px 40px", background: "linear-gradient(180deg, var(--eoc-navy-sh), var(--eoc-navy-sh-2))", boxShadow: "0 0 40px rgba(0,0,0,0.6)" }} />
            ...
            <div dir="ltr" style={{ position: "absolute", left: 20, bottom: 16, ...labelStyle }}>
              ...
              <div style={{ fontSize: 11, letterSpacing: "0.4em", color: "rgba(var(--eoc-primary), 0.75)", marginTop: 6 }}>EDITORIAL &amp; STUDIO</div>
            </div>
            <div style={{ position: "absolute", inset: 0, borderRadius: 14, boxShadow: "inset 0 0 60px rgba(var(--eoc-primary), 0.22), inset 0 0 0 1px rgba(var(--eoc-soft), 0.3)" }} />
          </div>
        </div>
        ...
        <div dir="ltr" style={v.servicesStyle}>
          ...
          <div style={{ width: 1, height: 14, background: "rgba(var(--eoc-accent), 0.4)" }} />
        ...
        <div style={v.hintStyle} hidden={v.hideHint}>
          ...
          <div style={{ width: 1, height: 34, margin: "14px auto 0", background: "linear-gradient(180deg, rgba(var(--eoc-accent), 0.8), transparent)" }} />
        </div>
